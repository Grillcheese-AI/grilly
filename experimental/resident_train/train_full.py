"""FULL TRAINING TEST — resident autograd, multi-step, train + held-out eval.
...
def grads(P, toks, labels):
    ...
    # NUMPY FORWARD (resident-forward path has a buffer-lifetime bug under the
    # multi-tape backward; backward is fully resident, which is the point here).
    # Compute every intermediate in numpy, exactly mirroring the resident graph,
    # to seed the resident backward tape.
    n1=(embf*P['w1']/np.sqrt((embf*embf).mean(1,keepdims=True)+EPS)).astype(np.float32)
    Gp=(n1@P['WG'].T).astype(np.float32); Vp=(n1@P['WV'].T).astype(np.float32); Dp=(n1@P['WD'].T).astype(np.float32)
    H=mingru_fwd(Gp.reshape(b,L,Hd),Vp.reshape(b,L,Hd),Dp.reshape(b,L,Hd))
    Hf=H.reshape(BL,Hd).astype(np.float32)
    r1=(emb+H); r1f=r1.reshape(BL,Hd).astype(np.float32)
    n2=(r1f*P['w2']/np.sqrt((r1f*r1f).mean(1,keepdims=True)+EPS)).astype(np.float32)
    gate=(n2@P['Wg'].T).astype(np.float32)
    x1=gate[:,:Hd]; x2=gate[:,Hd:]; ff=(x1*(x2*sig(x2))).astype(np.float32)
    r2=(r1+ff.reshape(b,L,Hd)); pooled=r2.mean(1).astype(np.float32)
    logits=(pooled@P['Whead'].T).astype(np.float32)
    n1r=n1; n2r=n2

    # --- backward tape (head) : Linear + CE on pooled ---
    t=gc.TapeContext(dev); t.begin()
    Wh=t.register_input(P['Whead'],True)
