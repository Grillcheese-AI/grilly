...
CHECKPOINT_PATTERN = re.compile(r'\b(?:checkpoint|ckpt|epoch|step|line|iteration)[\s_]*[\d,._]+', re.IGNORECASE)
...
def clean_text(text: str) -> str:
    ...
    text = re.sub(r'(\[(?:FILE|PATH|NUM|DIM|CHECKPOINT|PROJECT_NAME)\]\s*){3,}', '[REDACTED] ', text)
...
def main():
    ...
    with open(INPUT_FILE, encoding="utf-8") as fin, \
        ...
        for line in fin:
            ...
            if "lemmas" in entry:
                entry["lemmas"] = [clean_text(l) if LEAKED_NAMES.search(l) else l for l in entry["lemmas"]]
