...
def relu_backward(grad_output, input):
    ...
    try:
        return _core.relu_backward(
            dev, _ensure_f32_contiguous(grad_output),
...
def lif_step(input, v_mem, t_refrac, dt=1.0, tau_mem=20.0, v_rest=0.0,
             v_reset=0.0, v_thresh=1.0, r_mem=1.0, t_refrac_period=0.0):
    ...
    try:
        return _core.lif_step(
            dev, _ensure_f32_contiguous(input),
            _ensure_f32_contiguous(v_mem),
            _ensure_f32_contiguous(t_refrac),
            dt, tau_mem, v_rest, v_reset, v_thresh, r_mem, t_refrac_period)
    except Exception:
        return None


def snn_node_forward(x_in, v_mem, tau_param, neuron_type=1, tau=2.0,
    ...
    try:
        return _core.snn_node_forward(
            dev, _ensure_f32_contiguous(x_in),
            _ensure_f32_contiguous(v_mem),
            _ensure_f32_contiguous(tau_param),
            neuron_type, tau, v_threshold, v_reset, reset_mode, decay_input)
    except Exception:
        return None


def snn_node_backward(grad_spike, h_cache, alpha=2.0, surrogate_type=0,
                      v_threshold=1.0):
    ...
    try:
        return _core.snn_node_backward(
            dev, _ensure_f32_contiguous(grad_spike),
            _ensure_f32_contiguous(h_cache),
            alpha, surrogate_type, v_threshold)
    except Exception:
        return None


def hebbian_learning(pre, post, weights, batch_size=1, time_steps=1,
    ...
    try:
        return _core.hebbian_learning(
            dev, _ensure_f32_contiguous(pre),
            _ensure_f32_contiguous(post),
            _ensure_f32_contiguous(weights),
            batch_size, time_steps, learning_rate, weight_decay)
    except Exception:
        return None


def stdp_learning(pre, post, weights, pre_trace, post_trace,
                  batch_size=1, time_steps=1,
                  lr_pot=0.01, lr_dep=0.01, trace_decay=0.95):
    ...
    try:
        return _core.stdp_learning(
            ...
            batch_size, time_steps, lr_pot, lr_dep, trace_decay)
    except Exception:
        return None
...
def synapse_filter(x_in, y_state, decay=0.95):
    ...
    try:
        return _core.synapse_filter(
            dev, _ensure_f32_contiguous(x_in),
            _ensure_f32_contiguous(y_state), decay)
    except Exception:
        return None


def gif_neuron_step(input, v_mem, i_adapt, g_input, g_forget,
                    t_refrac, t_last_spike, dt=1.0, current_time=0.0,
                    tau_mem=20.0, v_rest=0.0, v_reset=0.0, v_thresh=1.0,
                    r_mem=1.0, tau_adapt=100.0, delta_adapt=0.1,
                    b_adapt=0.0, tau_gate=50.0, gate_strength=1.0,
                    t_refrac_period=0.0):
    ...
    try:
        return _core.gif_neuron_step(
            ...
            dt, current_time, tau_mem, v_rest, v_reset, v_thresh,
...
def tanh_backward(grad_output, tanh_output):
    ...
    try:
        return _core.tanh_backward(
            dev, _ensure_f32_contiguous(grad_output),
            _ensure_f32_contiguous(tanh_output))
    except Exception:
        return None
...
def softmax_backward(grad_output, softmax_output):
    ...
    try:
        return _core.softmax_backward(
            dev, _ensure_f32_contiguous(grad_output),
            _ensure_f32_contiguous(softmax_output))
    except Exception:
        return None
...
def dropout(x, random_mask, p=0.5, training=True):
    ...
    try:
        return _core.dropout(
            dev, _ensure_f32_contiguous(x),
            _ensure_f32_contiguous(random_mask), p, training)
    except Exception:
        return None
...
def conv2d_backward_input(grad_output, weight, input_shape,
    ...
    try:
        return _core.conv2d_backward_input(
            ...
            list(input_shape), list(stride), list(padding),
            list(dilation), groups)
    except Exception:
        return None


def conv2d_backward_weight(grad_output, input, weight_shape,
                           stride=(1, 1), padding=(0, 0),
    ...
    try:
        return _core.conv2d_backward_weight(
            dev, _ensure_f32_contiguous(grad_output),
            _ensure_f32_contiguous(input),
            list(weight_shape), list(stride), list(padding),
            list(dilation), groups, has_bias)
    except Exception:
        return None
...
def attention_scores(Q, K, scale=0.0):
    ...
    try:
        return _core.attention_scores(
            dev, _ensure_f32_contiguous(Q),
            _ensure_f32_contiguous(K), scale)
    except Exception:
        return None
...
def attention_mask(scores, mask=None, causal=True, mask_value=-1e9):
    ...
    try:
        ...
        return _core.attention_mask(
            dev, _ensure_f32_contiguous(scores), m, causal, mask_value)
    except Exception:
        return None
...
def attention_output(weights, V):
    ...
    try:
        return _core.attention_output(
            dev, _ensure_f32_contiguous(weights),
...
def attention_concat_heads(mh_output):
    ...
    if dev is None:
        return None
    try:
        return _core.attention_concat_heads(
            dev, _ensure_f32_contiguous(mh_output))
    except Exception:
        return None
...
def flash_attention2(Q, K, V, mask=None, scale=0.0, tile_size_q=64,
                     tile_size_k=64):
    ...
    try:
        ...
        m = _ensure_f32_contiguous(mask) if mask is not None else None
        return _core.flash_attention2(dev, Q, K, V, m, scale,
...
def maxpool2d(x, kernel_size, stride=(2, 2), padding=(0, 0), dilation=(1, 1)):
    ...
    try:
        return _core.maxpool2d(
            dev, _ensure_f32_contiguous(x),
            list(kernel_size), list(stride), list(padding), list(dilation))
    except Exception:
        return None


def avgpool2d(x, kernel_size, stride=(2, 2), padding=(0, 0),
    ...
    try:
        return _core.avgpool2d(
            dev, _ensure_f32_contiguous(x),
            list(kernel_size), list(stride), list(padding), count_include_pad)
    except Exception:
        return None
...
def batchnorm2d_forward(x, gamma, beta, running_mean, running_var,
...
def cross_entropy_backward(logits, targets):
    ...
    try:
        ...
        if targets.dtype != np.uint32:
            targets = targets.astype(np.uint32)
        return _core.cross_entropy_backward(
            dev, _ensure_f32_contiguous(logits), targets)
    except Exception:
        return None
...
def adam_update(weights, grad, m, v, lr=1e-3, beta1=0.9, beta2=0.999,
               eps=1e-8, beta1_t=0.9, beta2_t=0.999, clear_grad=False):
    ...
    try:
        return _core.adam_update(
            ...
            _ensure_f32_contiguous(m), _ensure_f32_contiguous(v),
            lr, beta1, beta2, eps, beta1_t, beta2_t, clear_grad)
    except Exception:
        return None


def adamw_update(weights, grad, m, v, lr=1e-3, beta1=0.9, beta2=0.999,
                 eps=1e-8, weight_decay=0.01,
                 beta1_t=0.9, beta2_t=0.999, clear_grad=False):
    ...
    try:
        return _core.adamw_update(
            ...
            _ensure_f32_contiguous(grad),
            _ensure_f32_contiguous(m), _ensure_f32_contiguous(v),
            lr, beta1, beta2, eps, weight_decay, beta1_t, beta2_t, clear_grad)
    except Exception:
        return None
...
def embedding_lookup(token_ids, embeddings):
    ...
    try:
        ...
        return _core.embedding_lookup(
            dev, token_ids, _ensure_f32_contiguous(embeddings))
    except Exception:
        return None
...
def create_kv_cache(max_seq_len, num_heads, head_dim, num_layers,
                    compression_ratio=1, max_cache_tokens=0,
                    use_asymmetric_quant=False, value_bits=16,
                    cross_layer_sharing=False, use_h2o=False,
                    use_speculative_eviction=False, eviction_threshold=0.1):
    ...
    try:
        ...
        return _core.create_kv_cache(
            dev, max_seq_len, num_heads, head_dim, num_layers,
            compression_ratio, max_cache_tokens,
            use_asymmetric_quant, value_bits, cross_layer_sharing,
            use_h2o, use_speculative_eviction, eviction_threshold)
    except Exception:
        return None
